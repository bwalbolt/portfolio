...
Use the available user-question tool, or ask concise direct questions if no such tool is available, to turn vague feedback into testable requirements.
...
Follow the evaluation steps in .agents/skills/harness/references/evaluator-guide.md exactly.
...
For this portfolio, the verification command is:

```bash
npm run verify
```

It runs lint, TypeScript, a production build, and Playwright smoke tests. Frontend work does not require TDD by default; add or update tests for navigation/user flows, accessibility behavior, complex state, data transformation, or regressions captured by acceptance criteria.
