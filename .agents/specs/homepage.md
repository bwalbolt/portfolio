...
- Use `figma-mcp-bridge` / desktop MCP during implementation to pull section-level design context from `Homepage` and `Homepage - Mobile`, because the remote MCP in this session is rate-limited.
...
- Figma remote MCP is currently rate-limited in this session, so implementation should rely on the desktop bridge for detailed section extraction before coding begins.
