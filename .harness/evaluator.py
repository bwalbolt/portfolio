...
def build_eval_prompt(plan: dict, task: dict, auto_fix: bool) -> str:
    ...
    return textwrap.dedent(
        ...
        Grep the task's implementation files for unfinished placeholders such as
        `TODO`, `FIXME`, `throw new Error("Not implemented")`,
        `NotImplementedError`, or empty stub functions. Do not fail ordinary
        JSX/TypeScript spread syntax (`...props`) or user-facing placeholder copy
        such as form placeholder text.
...
def main():
    ...
    if result.returncode != 0:
        print(f"Evaluator session failed (exit code {result.returncode})")
        if result.stderr:
            print(result.stderr)
        sys.exit(1)
